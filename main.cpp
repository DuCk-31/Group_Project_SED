...
bool isValid(vector <string> validOptions, string choice){
    for (int i = 0; i < validOptions.size(); i++){
        if (validOptions[i] == choice) return 1;
    }
    return 0;
}
...
void memberRegister(map <string, Member> &members){
    string usernameTemp;
    Member temp;
    cout << "Enter your username: "; getline(cin, usernameTemp);
    while (members.count(usernameTemp) == 1) {
        cout << "The username has been taken" << endl;
        cout << "Please enter another username: "; getline(cin, usernameTemp);
    }
...
int main(){
    ...
    while(true){
        ...
        cout << "Please select again: "; cin >> choice;
    }
    ...
    if (choice == "1"){
        ...
        cout << "Do you want to register: 1. Yes   2. No" << endl;
        cout << "Your choice: "; cin >> checkRegister;

        while (true) {
            if (checkRegister == "1" || checkRegister == "2") break;
            cout << "Invalid option, please select again: "; cin >> checkRegister;
        }
        ...
        while (members.count(usernameTemp) == 0) {
            wrongTimes++;
            cout << "Wrong username, please enter again: "; getline(cin, usernameTemp);
        ...
        wrongTimes = 0;
        cout << "Enter your password: "; getline(cin, passwordTemp);
        while (members[usernameTemp].verifiedPassword(passwordTemp) == 0){
            wrongTimes++;
            cout << "Wrong password, please enter password again: "; getline(cin, passwordTemp);
            if (wrongTimes == 5) {
        ...
        cout << endl << "Account overview: " << usernameTemp << endl;
        cout << "---------------------------------------" << endl << endl;
        ...
        cout << endl << "This is your menu: " << endl;
        cout << "1. Log out" << endl;
        ...
        while (true){
            cout << "Enter your choice: "; cin >> functionChoice; cin.ignore();
            if (functionChoice == "1") {
                break;
            }
                ...
                cout << "Enter start date: "; start.insertDate();
                cout << "Enter end date: "; end.insertDate();
                cout << "Enter location: "; cin >> location; cin.ignore();
                for (pair <string, Member> member : members){
                    if (member.second.checkCondition(start, end, members[usernameTemp].calculateRating(), members[usernameTemp].getCPs(), location) == 1
                        && member.second.checkOverlap(start, end) == 0 && member.first != usernameTemp){
                            if (!members[usernameTemp].checkLicense(end)){
                                if (member.second.getBike().checkSize()) {continue;}
                ...
                if (validOptions.empty()) {
                    ...
                    string choice;
                    cout << "Enter the username of member you want to send request: "; getline(cin, choice);
                    while (!isValid(validOptions, choice)){
                        cout << "Invalid option, please select again: "; getline(cin, choice);
                    }
                ...
                cout << "---------------------------------------" << endl;
                members[usernameTemp].showRequest();
                cout << "Enter the name of renter you want to accept: "; getline(cin, choice);
                while (members[usernameTemp].validateOptions(choice) == 0){
                    cout << "Invalid option, please enter again: "; getline(cin, choice);
                }
                ...
                cout << "---------------------------------------" << endl;
                members[usernameTemp].showRequest();
                cout << "Enter the name of renter you want to reject: "; getline(cin, choice);
                while (members[usernameTemp].validateOptions(choice) == 0){
                    cout << "Invalid option, please enter again: "; getline(cin, choice);
                }
